...
const std::string& file_contents() {
    static const std::string contents =
        load_file_as_string(std::string(g_host_info->data_dir) + FileSubPath);
    return contents;
}

/// Primitive templating engine
std::string replace_one(std::string haystack, const std::string_view needle,
...
std::string replace_all(std::string haystack, const std::string_view needle,
                        const std::string_view replacement) {
    std::size_t i = 0;
...
std::string index_html(const std::string& user) {
    ...
    return replace_one(file_contents<path>(), "{{contacts_json}}",
