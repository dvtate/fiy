...
struct Response : public fiy_response_t {
    explicit Response(int status, const char* body, std::size_t body_len,
...
struct Request : public fiy_request_t {
    ...
    explicit Request(const char* path, uint8_t method = (uint8_t)Method::GET,
                     const char* domain = nullptr, const char* user = nullptr,
     ...
     */
    inline void respond(fiy_callback_t cb, int status = 200, const std::string_view body = "",
                        const std::string_view headers = "") {
        fiy_response_t res = {.status = status,
                              .body = body.empty() ? nullptr : body.data(),
                              .body_len = body.size(),
                              .headers = headers.empty() ? nullptr : headers.data()};
        cb(this, &res);
    }
    inline void respond(fiy_callback_t cb, const std::string_view body,
                        const std::string_view headers = "") {
