...
void Peers::new_peer(const std::string& domain,
                     std::function<void(const std::shared_ptr<Peer>&)> cb) {
    DEBUG_LOG("Sending message to new peer " << domain);

    auto with_key_cb = [this, domain, cb = std::move(cb)](
        ...
        auto handshake_cb =
            [this, cb2 = std::move(cb), domain, token = std::move(tok)](
                boost::beast::http::response<boost::beast::http::string_body> res) mutable {
                // Make Peer
                auto p = std::make_shared<Peer>(
                    domain, PeerAuth("symkey", std::string(res.body()), std::move(token)));
...
struct ScopedRequest {
    ...
    explicit ScopedRequest(const fiy_request_t* req) {
        ...
        m_body = req->body == nullptr ? "" : std::string(req->body, req->body_len);
        m_method = req->method;
    }

    [[nodiscard]] fiy_request_t temp_copy() const {
        return fiy_request_t{.domain = m_domain.empty() ? nullptr : m_domain.c_str(),
...
void Peers::request_peer(const std::string& domain, const std::string& appid,
    ...
    if (p == nullptr) {
        ...
        new_peer(domain, [appid, request = ScopedRequest(req), callback,
                          context](const std::shared_ptr<Peer>& p) {
            if (p != nullptr) {
...
void Peers::request_peer(const std::shared_ptr<Peer>& peer, const std::string& appid,
                         const fiy_request_t* req, void* context,
                         void (*callback)(const fiy_response_t*, void*)) {
    // Null check
    if (peer == nullptr) {
    ...
    request.body() = std::string(
        req->body, req->body_len);  // note this should work even if body contains null chars
    ...
    auto cb = [context,
               callback](boost::beast::http::response<boost::beast::http::string_body> res) {
