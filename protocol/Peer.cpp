...
PeerAuth::PeerAuth(std::string sym_key, std::string peer_provided_token, std::string our_generated_token):
    PeerAuth(std::move(sym_key), std::move(peer_provided_token), std::move(our_generated_token), g_app->now() + SESSION_LIFETIME)
{}
...
std::string Peer::sig(
    ...
    const std::string& ts_str = std::to_string(g_app->now())
) const {
