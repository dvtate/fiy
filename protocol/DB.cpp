...
namespace DB {
    ...
    static SQLite::Statement& get_user_by_username_query(const std::string& username) {
    ...
    std::shared_ptr<Peer> get_peer(const std::string_view& domain) {
        static thread_local query{connection(), "SELECT * FROM Peers WHERE domain = ?"};
        CleanupRoutine cleanup{[&](){
            query.clearBindings();
            query.reset();
        }};

        query.bindNoCopy(1, std::string(domain));
        if (!query.executeStep()) // no match
            return nullptr;
