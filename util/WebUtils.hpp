 ...
 */
bool parse_form_url_encoded(const std::string_view s,
                            std::deque<std::pair<std::string, std::string>>& ret);
...
struct CookieOptions {
    ...
    /**
     * Specifies the `number` (in seconds) to be the value for the [`Max-Age` `Set-Cookie`
     ...
     * attribute](https://tools.ietf.org/html/rfc6265#section-5.2.1). When no expiration is set
     * clients consider this a "non-persistent cookie" and delete it when the current session is
     * over.
     *
     * The [cookie storage model specification](https://tools.ietf.org/html/rfc6265#section-5.3)
     * states that if both `expires` and `maxAge` are set, then `maxAge` takes precedence, but it is
     * possible not all clients by obey this, so if both are set, they should point to the same date
     * and time.
     */
    std::string expires;
     ...
     * attribute](https://tools.ietf.org/html/rfc6265#section-5.2.3). When no domain is set clients
     * consider the cookie to apply to the current domain only.
     */
    std::string domain;

    /**
     * Specifies the value for the [`Path` `Set-Cookie`
     * attribute](https://tools.ietf.org/html/rfc6265#section-5.2.4). When no path is set, the path
     * is considered the ["default path"](https://tools.ietf.org/html/rfc6265#section-5.1.4).
     */
    std::string path;

    /**
     * Enables the [`HttpOnly` `Set-Cookie`
     * attribute](https://tools.ietf.org/html/rfc6265#section-5.2.6). When enabled, clients will not
     * allow client-side JavaScript to see the cookie in `document.cookie`.
     */
    bool http_only{false};

    /**
     * Enables the [`Secure` `Set-Cookie`
     * attribute](https://tools.ietf.org/html/rfc6265#section-5.2.5). When enabled, clients will
     * only send the cookie back if the browser has a HTTPS connection.
     */
    bool secure{false};

    /**
     * Enables the [`Partitioned` `Set-Cookie`
     * attribute](https://tools.ietf.org/html/draft-cutler-httpbis-partitioned-cookies/). When
     * enabled, clients will only send the cookie back when the current domain _and_ top-level
     * domain matches.
     *
    ...
    /**
     * Specifies the value for the [`Priority` `Set-Cookie`
     * attribute](https://tools.ietf.org/html/draft-west-cookie-priority-00#section-4.1).
     *
     * - `'low'` will set the `Priority` attribute to `Low`.
     * - `'medium'` will set the `Priority` attribute to `Medium`, the default priority when not
     * set.
     * - `'high'` will set the `Priority` attribute to `High`.
     *
     * More information about priority levels can be found in [the
     * specification](https://tools.ietf.org/html/draft-west-cookie-priority-00#section-4.1).
     */
    std::string priority;

    /**
     * Specifies the value for the [`SameSite` `Set-Cookie`
     ...
     *
     * More information about enforcement levels can be found in [the
