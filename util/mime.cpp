...
/**
 * Guess the mime-type from the extension of a file
 */
std::string_view get_ext_mime_type(std::string ext) {
    // Source: https://github.com/jshttp/mime-db
    // Alternatively we could use libmagic but ehh
    static const boost::unordered_flat_map<std::string_view, std::string_view> mime_db{
    ...
    const auto it = mime_db.find(ext);
    if (it == mime_db.end())
